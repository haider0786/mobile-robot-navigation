...
cr2_z_model = load_model( '/home/haider/PycharmProjects/dy_multistepLSTM/cr2_A.h5')
cr2_x_model = load_model('/home/haider/PycharmProjects/dy_multistepLSTM/cr2_L.h5')
sr2_z_model = load_model( '/home/haider/PycharmProjects/dy_multistepLSTM/mlstm_A.h5')
sr2_x_model = load_model('/home/haider/PycharmProjects/dy_multistepLSTM/mlstm_L.h5')
clsfy_model = load_model('/home/haider/PycharmProjects/classification/classification.h5')
    ...
    try:
        ...
        if counter_o > 2:
            ...
            if counter_in == time_step:      ####################### counter_in = Np
                ...
                Input_pos1 = np.array(Input_pos1)
                Input_w = norm_laser(Input_w)
                Input_cls = Input_w
                Input_cls_3d = Input_cls.reshape((1, 306, 10))
                Input_w_3d = Input_w.reshape((1, time_step, 1020))
                Input_pos_3d = Input_pos.reshape((1, time_step, 2))
                Input_pos1_3d = Input_pos1.reshape(1, 2)

                cls = clsfy_model.predict(Input_cls_3d)
                cls = cls.argmax()
                ...
                if cls==1:
                    print("cls_sr2", cls)
                    angular_velocity = sr2_z_model.predict([Input_w_3d, Input_pos1_3d])
                    linear_velocity = sr2_x_model.predict([Input_w_3d, Input_pos1_3d])
                    if linear_velocity > 0.8:
                        linear_velocity = 0.8
                else:
                    print("cls_cr2", cls)
                    angular_velocity = cr2_z_model.predict([Input_w_3d, Input_pos1_3d])
                    linear_velocity = cr2_x_model.predict([Input_w_3d, Input_pos1_3d])
                    if linear_velocity > 0.8:
                        linear_velocity = 0.8 
