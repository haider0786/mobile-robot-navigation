...
obs_inf = []
Input_difflas = []
z_model = load_model('/home/robita/PycharmProjects/classification/srcrstvellsdif_A2.h5')
x_model = load_model('/home/robita/PycharmProjects/classification/srcrstvellsdiff_L1.h5')
def newOdom(msg):
    global laser
    laser = msg.ranges
...
while True:
    try:
        if counter_o > 2:
            ...
            if counter_diff < 6:
                #t_queue.append(counter_diff)
                v_laser.append(value1)
                print("counter_diff", counter_diff)
                counter_diff = counter_diff + 1
            else:
                print("counter_in", counter_in)
                #print("t_queue", t_queue.pop())
                print("value1_shape", np.array(value1).shape)
                v_diflas = np.array(value1)-np.array(v_laser.pop())
                print("v_diflas_shape", np.array(v_diflas.shape))
                Input_difflas.append(v_diflas)
            ...
            if counter_in == time_step:
                ...
                Input_difflas = np.array(Input_difflas)
                Input_difflas[Input_difflas==inf] = 0
                Input_difflas = norm_difflaser(Input_difflas)
                Input_difflas_3d = Input_difflas.reshape((1, time_step, 1020))
                ...
                angular_velocity = z_model.predict([Input_w_3d, Input_obs_info, Input_difflas_3d, Input_pos_3d])
                linear_velocity = x_model.predict([Input_w_3d, Input_obs_info, Input_difflas_3d, Input_pos_3d])
